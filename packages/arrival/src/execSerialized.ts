 ...
 * 
 * @param expr - The LIPS expression to execute
...
export async function execSerialized(
  ...
  // Wrap the expression in (list ...) to get all results as a list
  const wrappedExpr = `(list ${expr})`;
  
  // Execute with LIPS using sandboxed environment by default
  const execOptions = {
    env: sandboxedEnv,
    ...options
  };
  const result = await exec(wrappedExpr, execOptions);
...
function convertPairToArray(pair: any): any[] {
  ...
  if (current && current.constructor?.name !== "Nil" && 
